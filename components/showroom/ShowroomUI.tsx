...
type Props = {
  ...
  activo: ShowroomVehicle;
  onSeleccionar: (v: ShowroomVehicle) => void;
  onVista: (view: ShowroomView) => void;
};

export default function ShowroomUI({ marca, modelos, activo, onSeleccionar, onVista }: Props) {
  return (
          ...
          {modelos.map((m) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-xs font-medium transition ${
                m.id === activo.id ? "bg-white text-black" : "bg-white/10 hover:bg-white/20"
              }`}
            >
              {m.nombre}
            </button>
