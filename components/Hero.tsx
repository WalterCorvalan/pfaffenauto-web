...
export default function Hero() {
  ...
  return (
    <section 
      ...
      className="relative w-full pt-20 pb-24 md:pt-32 md:pb-32 overflow-hidden flex flex-col items-center justify-center border-b border-gray-200/50 bg-black"
    >
      ...
      <div className="absolute inset-0 bg-[#F8FAFC]/75 backdrop-blur-[3px] z-0"></div>
      ...
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#e2e8f0_1px,transparent_1px),linear-gradient(to_bottom,#e2e8f0_1px,transparent_1px)] bg-[size:3rem_3rem] [mask-image:radial-gradient(ellipse_60%_60%_at_50%_40%,#000_70%,transparent_100%)] z-0 opacity-50"></div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 flex flex-col items-center text-center">
        ...
        <motion.div 
          ...
          className="flex items-center gap-2 bg-white/60 backdrop-blur-md border border-white/80 text-navy font-bold text-[10px] md:text-[11px] uppercase tracking-widest px-5 py-2 rounded-full mb-8 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)]"
        >
        ...
        <motion.form 
          ...
          className="w-full max-w-3xl bg-white/70 backdrop-blur-3xl rounded-3xl shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] border border-white p-2 flex flex-col md:flex-row items-center gap-2 mb-10 group transition-all hover:bg-white/90"
        >
          <div className="flex items-center w-full px-4 py-3 md:py-2 relative z-10">
            ...
            <input 
              ...
              className="w-full bg-transparent border-none outline-none text-navy placeholder:text-gray-500 text-base md:text-lg font-medium"
              value={searchTerm}
...
function Pill({ icon, text, borderClass = "border-white bg-white/60 hover:bg-white/90 text-navy", href = "/catalogo" }: { icon: React.ReactNode, text: string, borderClass?: string, href?: string }) {
  return (
    <Link href={href}>
      <div className={`backdrop-blur-md border shadow-[0_4px_12px_rgba(0,0,0,0.04)] rounded-2xl px-5 py-3 flex items-center gap-2.5 text-[13px] font-black tracking-wide transition-all duration-300 hover:-translate-y-1 hover:shadow-lg cursor-pointer ${borderClass}`}>
        {icon}
        <span>{text}</span>
      </div>
    </Link>
