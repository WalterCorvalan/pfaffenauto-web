...
export default function FirmaCanvas({
...
}: { tabla: string; id: string; firmaUrlActual: string | null; onGuardada: (url: string) => void; campo?: string }) {
  ...
  const guardarFirma = async () => {
    ...
    try {
      ...
      const { error: dbError } = await supabase2.from(tabla).update({ [campo]: data.publicUrl }).eq("id", id);
  ...
  return (
    <div className="print:hidden">
      <canvas
        ref={canvasRef}
        className="w-full h-24 bg-slate-50 dark:bg-white/5 border-2 border-dashed border-slate-300 dark:border-white/10 rounded-xl touch-none cursor-crosshair"
