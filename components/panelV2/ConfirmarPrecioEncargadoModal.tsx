...
const inputClass = "w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-rose-500 focus:bg-white dark:focus:bg-white/10 transition-colors text-slate-900 dark:text-white";

// El encargado revisa el precio que cargó el vendedor: lo deja igual
// (confirma) o lo corrige. Queda registro de qué valor final quedó, para
// poder avisarle al vendedor qué contestó.
export default function ConfirmarPrecioEncargadoModal({
  precioArsActual, precioUsdActual, guardando, onConfirmar, onClose,
}: { precioArsActual: number | null; precioUsdActual: number | null; guardando: boolean; onConfirmar: (ars: number | null, usd: number | null) => void; onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-[#141414] border border-slate-200 dark:border-white/10 rounded-2xl p-6 w-full max-w-sm space-y-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"><X className="w-4 h-4" /></button>
        </div>
