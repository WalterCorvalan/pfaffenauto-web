...
- **Precio/tipo/condición**: filtros directos sobre columnas de `vehiculos`. La condición "0KM vs usado" filtra por `km === 0` (**no** por la columna `condicion`, que quedó sin uso real — el formulario de Stock, `NuevoVehiculoModal.tsx`, no tiene selector de condición y nunca la escribe). Mismo criterio en `/0km`, `ComparadorModal.tsx`, `lib/showroom/mapear.ts`, `buscar-ia/route.ts` y `lib/ads/mercadolibrePublish.ts`. `km` es obligatorio al cargar/editar un vehículo en `NuevoVehiculoModal.tsx`, así que no debería quedar en 0/vacío por un usado sin cargar — si eso vuelve a pasar (un usado apareciendo en `/0km`), el problema es un `km` mal cargado en Stock, no el filtro.
