...
import { Search, ChevronDown, MapPin, SlidersHorizontal, X, Filter, Scale, Loader2 } from "lucide-react";
...
export default function CatalogoPage() {
  ...
  const fetchVehiculos = async (pageIndex: number, reemplazar: boolean = false) => {
    ...
    if (searchQuery) query = query.or(`marca.ilike.%${searchQuery}%,modelo.ilike.%${searchQuery}%`);
    ...
    if (tiposSeleccionados.length > 0) query = query.in("tipo", tiposSeleccionados);
    ...
    if (data) {
      if (reemplazar) {
        ...
        // Filtramos para asegurar que NUNCA se metan IDs repetidos en el state
        setVehiculos(prev => {
          const idsExistentes = new Set(prev.map(item => item.id));
          const nuevosUnicos = data.filter(item => !idsExistentes.has(item.id));
  ...
  // Efecto cuando cambian los filtros (Resetea la página a 0 y pisa los datos)
  useEffect(() => {
    setPagina(0);
  ...
  const toggleComparar = (e: React.MouseEvent, auto: any) => {
    ...
    if (yaEsta) {
      ...
      if (autosComparar.length >= 2) setAutosComparar([autosComparar[1], auto]);
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="mb-6">
          <div className="text-xs text-slate-400 font-medium mb-1">
            <Link href="/" className="hover:text-[#0145F2] transition-colors">Inicio</Link> / <span className="text-slate-600">Catálogo</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-light text-navy tracking-tighter">
            Catálogo de autos <strong className="font-black text-transparent bg-clip-text bg-gradient-to-r from-[#0145F2] to-sky-400">0km y usados</strong>
          </h1>
        </div>
        ...
        <div className="bg-white/60 backdrop-blur-2xl border border-white rounded-3xl p-4 mb-6 shadow-[0_8px_32px_rgba(0,0,0,0.03)] flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs text-slate-500 font-bold uppercase tracking-wider mr-2">Filtros aplicados:</span>
            {searchQuery ? (
              <span className="inline-flex items-center gap-1.5 bg-white border border-sky-100 text-[#0145F2] shadow-sm text-xs font-bold px-4 py-1.5 rounded-full">
                ...
                <Link href="/catalogo" className="hover:text-red-500 transition-colors bg-sky-50 rounded-full p-0.5"><X className="w-3.5 h-3.5" /></Link>
              </span>
            ) : (
              <span className="text-xs text-slate-400 italic">Ningún filtro activo.</span>
            )}
          </div>
          {searchQuery && (
            <Link href="/catalogo" className="text-xs font-bold text-[#0145F2] hover:underline active:scale-95 transition-transform">
              Limpiar todo
            </Link>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 pb-4 border-b border-slate-200/50">
          ...
          <div className="flex items-center justify-between w-full sm:w-auto gap-4">
            <span className="text-sm font-medium text-slate-500">
              <strong className="text-navy font-black text-lg">{totalResultados}</strong> vehículos
            </span>
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            <label className="flex items-center gap-2 text-[10px] md:text-xs font-bold text-slate-600 cursor-pointer bg-white/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white shadow-[0_2px_10px_rgba(0,0,0,0.02)] hover:bg-white/80 transition-all">
              <span>Crédito BNA</span>
              <input type="checkbox" className="sr-only peer" />
              <div className="w-6 h-3.5 md:w-8 md:h-4 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-200 after:border after:rounded-full after:h-2.5 after:w-2.5 md:after:h-3 md:after:w-3 after:transition-all peer-checked:bg-[#0145F2] relative shadow-inner"></div>
            </label>
        ...
        {/* ================= DISEÑO DE 2 COLUMNAS (CONTENIDO PRINCIPAL) ================= */}
        <div className="flex flex-col lg:flex-row gap-4 md:gap-8">
          ...
          {isFilterOpen && (
            <div className="fixed inset-0 z-[60] flex lg:hidden">
              <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm" onClick={() => setIsFilterOpen(false)}></div>
              <motion.div 
              ...
              >
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-200/50">
                  <h3 className="text-sm font-black text-navy uppercase tracking-tight">Filtros</h3>
                  <button onClick={() => setIsFilterOpen(false)} className="p-1.5 text-slate-400 hover:text-navy bg-white rounded-full border border-slate-100 shadow-sm active:scale-90 transition-all">
                    <X className="w-4 h-4" />
                  </button>
                </div>
          ...
          <div className="flex-1 w-full flex flex-col">
            {loading ? (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-6">
                {[...Array(6)].map((_, i) => (
                  <div key={i} className="bg-white/40 backdrop-blur-xl rounded-[20px] sm:rounded-3xl border border-white overflow-hidden flex flex-col h-full shadow-sm p-3 sm:p-4 animate-pulse">
                    ...
                    <div className="flex flex-col flex-grow">
                      ...
                      <div className="h-2 sm:h-3 bg-slate-200 rounded-full w-1/2 mb-4 sm:mb-6"></div>
                      <div className="mt-auto pt-2 sm:pt-3">
                        <div className="bg-white/50 rounded-xl sm:rounded-2xl p-2 sm:p-3 mb-2 sm:mb-3 border border-white">
                          <div className="h-1.5 sm:h-2 bg-slate-200 rounded-full w-1/4 mb-2 sm:mb-3"></div>
                          <div className="h-4 sm:h-7 bg-slate-300 rounded-full w-2/3"></div>
                        </div>
              ...
              <>
                ...
                >
                    {vehiculos.map((auto, index) => {
                      ...
                      return (<motion.div key={`${auto.id}-${index}`}
                        variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.4, ease: "easeOut" } } }} className="h-full"
                      >
                        <Link href={`/catalogo/${auto.slug}`} className="block group h-full transition-transform duration-300 active:scale-[0.98]">
                          <div className={`bg-white/70 backdrop-blur-2xl rounded-[20px] sm:rounded-[28px] overflow-hidden flex flex-col h-full shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_15px_35px_rgba(1,69,242,0.08)] hover:-translate-y-1 transition-all duration-500 relative border
              ...
              <div className="text-center py-20 sm:py-24 bg-white/60 backdrop-blur-xl rounded-[20px] sm:rounded-3xl border border-white shadow-sm px-4">
                ...
                <Link href="/catalogo" className="inline-block bg-gradient-to-r from-[#0145F2] to-blue-600 hover:to-sky-500 active:scale-95 text-white font-bold text-[10px] sm:text-xs uppercase tracking-widest px-6 sm:px-8 py-3 sm:py-3.5 rounded-full transition-all shadow-lg shadow-blue-500/20">
                  Limpiar filtros
                </Link>
              </div>
            )}
              ...
              <div className="flex items-center gap-2 md:gap-3 shrink-0">
                <div className="flex -space-x-3">
                  {autosComparar.map((auto, i) => (
                    <div key={i} className="w-8 h-8 md:w-12 md:h-12 rounded-full border-2 border-[#0F172A] overflow-hidden bg-white shadow-md shrink-0">
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <span className="text-slate-400 text-[8px] md:text-[9px] uppercase tracking-widest hidden sm:block mt-1">Comparador</span>
                </div>
              </div>
...
function FiltrosContent({ 
  ...
  return (
    <div className="bg-white/60 backdrop-blur-2xl border border-white rounded-[24px] md:rounded-[28px] p-5 md:p-6 shadow-[0_8px_32px_rgba(0,0,0,0.03)]">
      <div className="flex items-center justify-between mb-5 md:mb-6 pb-3 md:pb-4 border-b border-slate-200/50">
      ...
      <div className="space-y-3 mb-6 md:mb-8">
        <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1 md:mb-2">Precio (ARS)</label>
      ...
      <div className="space-y-3 md:space-y-4 pt-5 md:pt-6 border-t border-slate-200/50">
        <label className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1 md:mb-2">Carrocería</label>
        {["Hatchback", "Pick-up", "Sedán", "SUV", "Utilitario"].map((tipo, idx) => (
          <label key={idx} className="flex items-center gap-2.5 md:gap-3 text-[11px] md:text-xs text-slate-600 font-bold cursor-pointer hover:text-navy transition-colors group">
            ...
            <span className="group-hover:translate-x-1 transition-transform">{tipo}</span>
          </label>
        ))}
      </div>
    </div>
