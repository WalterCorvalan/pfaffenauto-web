...
export default function VentasClient({
  ventasIniciales, perfiles, clientes, vehiculos, ventaIdsConPermuta, miId,
}: { ventasIniciales: Venta[]; perfiles: Perfil[]; clientes: Cliente[]; vehiculos: Vehiculo[]; ventaIdsConPermuta: string[]; miId: string }) {
  ...
  const filtradas = useMemo(() => {
    ...
    return lista;
  }, [ventas, tab, query, queryVendedor, metodoFiltro, desde, hasta, soloPermuta, mes, perfilMap, permutaSet]);
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 dark:bg-white/[0.03] border-b border-slate-200 dark:border-white/5">
                    <th className="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Comprador</th>
                    ...
                    <th className="px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Cierre</th>
                ...
                <tbody>
                  {filtradas.map((v) => (
                    <tr key={v.id} className="border-b border-slate-100 dark:border-white/5 last:border-0 hover:bg-slate-50 dark:hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-300">{[v.vehiculo_marca, v.vehiculo_modelo, v.vehiculo_anio].filter(Boolean).join(" ") || "—"}{v.vehiculo_patente ? ` · ${v.vehiculo_patente}` : ""}{permutaSet.has(v.id) && <span className="ml-1.5 text-[9px] font-bold px-1.5 py-0.5 rounded bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-300">PERMUTA</span>}</td>
                      <td className="px-4 py-3 text-sm font-bold text-slate-700 dark:text-slate-200">{v.moneda_venta} {v.precio_venta.toLocaleString("es-AR")}</td>
                      <td className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400">{v.vendedor_id ? perfilMap[v.vendedor_id] || "—" : "—"}</td>
                      <td className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400">{fmtFechaLocal(v.fecha_cierre)}</td>
                      <td className="px-4 py-3"><span className={`text-[10px] font-bold px-2 py-1 rounded-full border ${ESTADO_COLOR[v.estado]}`}>{v.estado}</span></td>
