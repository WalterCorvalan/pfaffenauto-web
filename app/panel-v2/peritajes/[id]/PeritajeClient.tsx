...
import {
  ArrowLeft, ClipboardCheck, CarFront, User, Paperclip, Loader2, CheckCircle2, Printer, Wrench, X,
} from "lucide-react";
...
const inputClass = "w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-[12px] text-slate-800 dark:text-white outline-none focus:border-rose-500 transition-colors placeholder:text-slate-400";
...
export default function PeritajeClient({ peritaje, itemsIniciales }: { peritaje: any; itemsIniciales: any[] }) {
  ...
  const [usoInterno, setUsoInterno] = useState({
    ...
    ok_dto_vu: peritaje.ok_dto_vu || false,
    ok_gerencia_ventas: peritaje.ok_gerencia_ventas || false,
  });
  ...
  const grupos = useMemo(() => {
    ...
    for (const it of items) {
      if (it.categoria === "Neumáticos") continue;
  ...
  const actualizarItem = async (id: string, cambios: Partial<{ estado: string; observacion: string; foto_url: string; necesita_reparacion: boolean; gastos_reparacion: number | null }>) => {
    ...
    const { error } = await supabase2.from("peritaje_lead_items").update(cambios).eq("id", id);
    if (error) console.error("Error guardando ítem de peritaje:", error);

    if (cambios.estado) {
      const nuevoPuntaje = calcularPuntaje(nuevosItems);
      await supabase2.from("peritajes_lead").update({ puntaje: nuevoPuntaje, updated_at: new Date().toISOString() }).eq("id", peritaje.id);
    }
  };
  ...
  const toggleAccesorio = async (clave: string) => {
    ...
    const { error } = await supabase2.from("peritajes_lead").update({ accesorios: nuevo }).eq("id", peritaje.id);
  ...
  const guardarUsoInterno = async () => {
    ...
    const { error } = await supabase2.from("peritajes_lead").update(payload).eq("id", peritaje.id);
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white dark:bg-[#141414] overflow-hidden">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/5 px-6 py-4 bg-white dark:bg-white/[0.02] shrink-0 print:hidden">
        <div className="flex items-center gap-4">
          <Link href="/panel-v2/peritajes" className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 p-2.5 rounded-lg border border-slate-200 dark:border-white/10 transition-colors">
          ...
          <div>
            ...
            <p className="text-[11px] font-medium text-slate-500 dark:text-slate-400 mt-0.5 flex items-center gap-1.5">
              <CarFront className="w-3 h-3" /> {peritaje.vehiculo_descripcion || "Sin descripción"}
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 p-2.5 rounded-xl transition-colors"
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-slate-50 dark:bg-[#141414]">
        <div className="max-w-3xl mx-auto space-y-5">
                        ...
                        {ESTADOS_ITEM_PERITAJE.map((e) => (
                          <button
                            ...
                            className={`text-[10px] font-bold uppercase tracking-widest px-2.5 py-1.5 rounded-lg border transition-colors ${
                              item.estado === e ? COLOR_ESTADO_ITEM[e] : "bg-white dark:bg-white/[0.02] border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-500"
                            }`}
                          >
                            {e}
                          </button>
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      {item.necesita_reparacion && (
                        <input
                          ...
                          className="w-32 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-2.5 py-1 text-[11px] text-slate-800 dark:text-white outline-none focus:border-rose-500 transition-colors"
                        />
                      )}
                    </div>
            ...
            <div className="bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl overflow-hidden">
              ...
              <div>
                {[
                  ...
                  <div key={grupo.titulo} className={i > 0 ? "border-t-4 border-slate-100 dark:border-white/10" : ""}>
                    <p className="px-4 pt-3 pb-1 text-[9px] font-black uppercase tracking-widest text-slate-400">{grupo.titulo}</p>
                    <div className="divide-y divide-slate-100 dark:divide-white/10">
                      {grupo.items.map((item: any) => (
                        <div key={item.id} className="p-4 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                          ...
                          <input
                            ...
                            onBlur={(e) => e.target.value !== (item.observacion || "") && actualizarItem(item.id, { observacion: e.target.value })}
                            placeholder="Marca..."
                            className="w-28 sm:flex-1 sm:min-w-[120px] bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-1.5 text-[12px] text-slate-800 dark:text-white outline-none focus:border-rose-500 transition-colors placeholder:text-slate-400"
                          />
                          <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <div className="bg-white dark:bg-white/[0.02] border border-amber-200 dark:border-amber-400/20 rounded-2xl overflow-hidden">
                  ...
                  <div className="flex gap-1.5">
                    {ESTADOS_ITEM_PERITAJE.map((e) => (
                      <button
                        ...
                        className={`flex-1 text-[10px] font-bold uppercase tracking-widest px-2 py-1.5 rounded-lg border transition-colors ${usoInterno.estado_general_vu === e ? COLOR_ESTADO_ITEM[e] : "bg-white dark:bg-white/[0.02] border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400"}`}
                      >
                        {e}
                      </button>
                    ))}
