...
export default function DiagramaCarroceria({ peritajeId, marcasIniciales }: { peritajeId: string; marcasIniciales: Record<string, string> }) {
  ...
  const elegirSimbolo = async (simbolo: string | null) => {
    ...
    setGuardando(true);
    const { error } = await supabase2.from("peritajes_lead").update({ carroceria_marcas: nuevo }).eq("id", peritajeId);
    ...
    <div className="bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl overflow-hidden">
      ...
      <div className="p-4 flex flex-wrap items-end justify-center gap-4 lg:grid lg:[grid-template-columns:repeat(auto-fill,minmax(260px,1fr))] lg:items-stretch lg:gap-5">
        {VISTAS.map((vista) => {
          ...
          return (
            <div key={vista.id} className="flex flex-col items-center w-full sm:w-auto lg:w-full lg:bg-slate-50 lg:dark:bg-white/5 lg:border lg:border-slate-100 lg:dark:border-white/10 lg:rounded-xl lg:p-3">
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 text-center mb-1">{vista.titulo}</p>
                    ...
                    return (
                      <button
                        ...
                        className={`absolute -translate-x-1/2 -translate-y-1/2 w-4 h-4 lg:w-5 lg:h-5 rounded-full border shadow-sm transition-all flex items-center justify-center ${
                          modoAjuste
                            ? "bg-rose-500/20 border-rose-500 cursor-grab active:cursor-grabbing"
                            : marcada
                              ? "bg-rose-500/20 border-rose-500"
                              : "bg-white/90 border-slate-400 hover:bg-rose-500/10 hover:border-rose-500"
                        }`}
      ...
      {popupAbierto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setPopupAbierto(null)}>
          ...
          <div
            className="relative bg-white dark:bg-[#141414] border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl p-5 w-full max-w-xs"
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              {SIMBOLOS.map((s) => (
                <button
                  ...
                  className="flex flex-col items-center gap-1 p-3 rounded-xl border border-slate-200 dark:border-white/10 hover:border-rose-400 dark:hover:border-rose-400/50 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors"
                >
            ...
            <button
              ...
              className="w-full mt-3 text-[11px] font-bold text-slate-400 hover:text-rose-500 dark:hover:text-rose-400 py-2 transition-colors"
            >
              Quitar marca
            </button>
          </div>
