...
**Bug corregido (barrido de Tesorería): `guardarConsignacion` y `agregarGasto` con el mismo problema.** Se habían quedado afuera de la ronda anterior. `guardarConsignacion` guarda `precio_propietario` — el campo que alimenta directo la Liquidación (honorarios, neto a pagar al propietario) — con `.single()` sin chequear `error`: un guardado fallido no mostraba ningún aviso. `agregarGasto` (tab Gastos, usado también desde `app/panel/tesoreria/expedientes/`) no chequeaba `error` en absoluto — si el insert en `expediente_gastos` fallaba, el formulario se limpiaba igual, como si el gasto se hubiera cargado, y el gasto (que afecta los totales de Liquidación/Tesorería) desaparecía sin rastro. Ambos arreglados con `.maybeSingle()` + chequeo explícito de `error`/`!data`, mismo patrón que el resto del archivo.

**Bug corregido: `guardarPagoComprador` podía dejar un estado a medias entre dos RPCs.** Hace dos llamados seguidos, sin transacción: `registrar_pago_comprador_venta` y `registrar_extra_cobrado_venta`. Si el primero aplicaba y el segundo fallaba, la venta quedaba con el pago del comprador confirmado pero sin el ingreso de "extra cobrado" en Finanzas — un estado inconsistente sin ningún aviso específico (solo un alert genérico). Ahora, si el segundo falla, se llama de nuevo a `registrar_pago_comprador_venta` con el snapshot del estado previo (tomado de `venta` antes de guardar) para revertir el primero. Si esa reversión también falla, el mensaje de error dice explícitamente qué quedó aplicado y qué no, para que se revise a mano en vez de asumir "no se guardó nada".
