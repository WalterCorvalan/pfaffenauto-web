...
export default function ExpedienteDetalleModal({ expedienteId, miId, perfiles, soyAdmin, puedeOperacionCaida, puedeVerLiquidacion, gananciasOcultas, tabInicial, onClose, onActualizado, onEliminado }: Props) {
  ...
  const [respuestaPedido, setRespuestaPedido] = useState("");
  const [respondiendoPedido, setRespondiendoPedido] = useState(false);
  ...
  // Responder un pedido de atención lo cierra: queda la respuesta en la
  // bitácora de observaciones (tipo "respuesta_pedido_atencion", para
  // distinguirla de una nota general) y se limpian los campos flat del
  // expediente que alimentan el banner indigo de arriba, así deja de
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={() => onClose()}>
      <div className="bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-5 pb-5 space-y-4">
          ...
          {expediente.pedido_atencion_sector && (
            <div className="bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 rounded-xl p-3">
              <p className="text-xs font-bold text-indigo-700 dark:text-indigo-300">🔔 Pedidos pendientes de atención (1)</p>
              <p className="text-xs text-indigo-700/80 dark:text-indigo-300/70 mt-1">A {SECTORES.find((s) => s.value === expediente.pedido_atencion_sector)?.label || expediente.pedido_atencion_sector}</p>
              ...
              <div className="flex items-center gap-2 mt-2">
                <input
                  value={respuestaPedido}
                  onChange={(e) => setRespuestaPedido(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && responderPedido()}
                  placeholder="Escribí tu respuesta..."
                  className="flex-1 bg-white dark:bg-white/5 border border-indigo-200 dark:border-indigo-500/20 rounded-lg px-2.5 py-1.5 text-xs outline-none focus:border-indigo-500 text-slate-900 dark:text-white"
                />
                <button onClick={responderPedido} disabled={!respuestaPedido.trim() || respondiendoPedido} className="px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold disabled:opacity-50 shrink-0">
                  {respondiendoPedido ? "..." : "Responder"}
                </button>
              </div>
            </div>
          )}
