...
Al eliminar una venta (o su expediente): si el vehículo vinculado (`ventas.vehiculo_id`) está en `'vendido'`, pasa a `'disponible'`. Al restaurar: vuelve a `'vendido'`. Es una interpretación de "estado anterior" simplificada a propósito — no hay ninguna tabla que registre el estado exacto del vehículo antes de la venta (podía ser `disponible`, `reservado` o `señado`), así que se asume `disponible` siempre al revertir. Si en algún momento se necesita precisión ahí, hay que empezar a guardar el estado previo en algún lado (ej. una columna en `ventas` al crearla), esto no lo hace.
