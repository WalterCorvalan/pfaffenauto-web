...
export default function CajaGrandeChicaTab({ miId, soyAdmin, cuentas, setCuentas, movimientos, setMovimientos, sucursales, vendedores }: Props) {
  ...
  // ajuste el saldo in-place, y este camino ya usa los dos RPCs que sí
  // manejan el saldo bien (incluye el bloqueo de mes cerrado del lado del
  // servidor). Solo para movimientos manuales/gasto -- una transferencia
  // (2 movimientos linkeados) queda fuera de este alcance.
  const [editando, setEditando] = useState<any>(null);
  const [eTipo, setETipo] = useState<"ingreso" | "egreso">("ingreso");
  const [eConcepto, setEConcepto] = useState("");
  const [eCategoria, setECategoria] = useState(CATEGORIAS_CAJA_CHICA[0]);
  const [eMonto, setEMonto] = useState("");
  const [eFecha, setEFecha] = useState("");
  const [eMedioPago, setEMedioPago] = useState("Efectivo");
  ...
  const abrirEditar = (m: any) => {
    setEditando(m);
    setETipo(m.tipo);
    setEConcepto(m.observaciones && CATEGORIAS_CAJA_CHICA.includes(m.tipo_movimiento) ? "" : (m.observaciones || ""));
    setECategoria(CATEGORIAS_CAJA_CHICA.includes(m.tipo_movimiento) ? m.tipo_movimiento : CATEGORIAS_CAJA_CHICA[0]);
    setEMonto(String(m.monto));
    setEFecha(m.fecha);
    setEMedioPago(m.forma_pago || "Efectivo");
    setEObs(m.observaciones || "");
  };

  const guardarEdicion = async () => {
    if (!editando || !cuenta || !eMonto || Number(eMonto) <= 0) return alert("Completá el monto.");
    setGuardandoEdicion(true);
    try {
      const esGasto = CATEGORIAS_CAJA_CHICA.includes(editando.tipo_movimiento);
      const { error: errorBorrar } = await supabase2.rpc("eliminar_movimiento_caja", { p_movimiento_id: editando.id, p_motivo: "Editado" });
      if (errorBorrar) throw errorBorrar;

      const categoria = esGasto ? eCategoria : (eConcepto || (eTipo === "ingreso" ? "Ingreso" : "Egreso"));
      const { data: movId, error: errorCrear } = await supabase2.rpc("registrar_movimiento_caja", {
        p_tipo: eTipo, p_monto: Number(eMonto), p_cuenta_id: cuenta.id, p_fecha: eFecha,
        p_categoria: categoria, p_forma_pago: eMedioPago, p_observaciones: eObs || eConcepto || null,
      });
      if (errorCrear) throw errorCrear;

      const { data: actualizado } = await supabase2
        .from("movimientos_caja")
        .update({ vendedor_id: miId, comprobante_url: editando.comprobante_url || null, observaciones: eObs || eConcepto || null })
        .eq("id", movId)
      ...
      setMovimientos((prev: any[]) => {
        ...
        return actualizado ? [actualizado, ...sinElAnterior] : sinElAnterior;
      });
      // Reversa el efecto del movimiento viejo y aplica el nuevo.
      setCuentas((prev: any[]) => prev.map((c) => {
        if (c.id !== cuenta.id) return c;
        const saldoSinViejo = (c.saldo || 0) - (editando.tipo === "ingreso" ? Number(editando.monto) : -Number(editando.monto));
        return { ...c, saldo: saldoSinViejo + (eTipo === "ingreso" ? Number(eMonto) : -Number(eMonto)) };
      }));
      setEditando(null);
    } catch (err: any) {
      alert(err.message || "No se pudo editar el movimiento (puede que el mes esté cerrado).");
    } finally {
  ...
  const registrar = async () => {
    ...
    try {
      ...
      // El RPC ya actualiza cuenta.saldo en la base -- sin esto, el estado
      // local de "cuentas" quedaba con el saldo viejo hasta recargar la
      // página, y "Saldo inicial" se calculaba mal (podía dar negativo).
      setCuentas((prev: any[]) => prev.map((c) => (c.id === cuenta.id ? { ...c, saldo: (c.saldo || 0) + (rTipo === "ingreso" ? Number(rMonto) : -Number(rMonto)) } : c)));
    ...
    <div>
      ...
      {editando && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setEditando(null)}>
          <div onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-[#141414] border border-slate-200 dark:border-white/10 w-full max-w-md rounded-2xl shadow-2xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-1"><h3 className="text-lg font-bold">Editar movimiento</h3><button onClick={() => setEditando(null)}><X className="w-4 h-4 text-slate-400" /></button></div>
            <p className="text-xs text-slate-400 mb-4">{cuenta?.nombre}</p>

            {CATEGORIAS_CAJA_CHICA.includes(editando.tipo_movimiento) && (
              <div className="mb-3"><label className={labelClass}>Categoría de gasto *</label>
                <select value={eCategoria} onChange={(e) => setECategoria(e.target.value)} className={inputClass}>
                  {CATEGORIAS_CAJA_CHICA.map((c) => <option key={c}>{c}</option>)}
                </select>
              </div>
            )}
            ...
            <input value={eConcepto} onChange={(e) => setEConcepto(e.target.value)} placeholder="Descripción del movimiento" className={inputClass} />

            <div className="grid grid-cols-2 gap-2 mt-3">
              <div><label className={labelClass}>Monto *</label><input type="text" inputMode="numeric" value={eMonto} onChange={(e) => setEMonto(e.target.value.replace(/\D/g, ""))} className={inputClass} /></div>
              <div><label className={labelClass}>Fecha *</label><input type="date" value={eFecha} onChange={(e) => setEFecha(e.target.value)} className={inputClass} /></div>
            </div>

            <div className="mt-3"><label className={labelClass}>Medio de pago</label>
              <select value={eMedioPago} onChange={(e) => setEMedioPago(e.target.value)} className={inputClass}>
                {MEDIOS_PAGO.map((m) => <option key={m}>{m}</option>)}
              </select>
            </div>

            <label className={labelClass + " mt-3"}>Observaciones</label>
            <textarea value={eObs} onChange={(e) => setEObs(e.target.value)} rows={2} className={inputClass} />

            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setEditando(null)} className="px-4 py-2 text-sm font-bold text-slate-500">Cancelar</button>
              <button
                onClick={() => guardarEdicion()}
                disabled={guardandoEdicion}
                className="flex items-center gap-1.5 px-4 py-2 text-sm font-bold bg-[#0145F2] hover:bg-[#0138c9] text-white rounded-lg disabled:opacity-50"
              >
                <Save className="w-4 h-4" /> {guardandoEdicion ? "Guardando..." : "Guardar cambios"}
              </button>
            </div>
          </div>
        </div>
