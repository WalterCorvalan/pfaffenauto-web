...
- **Eliminar una venta ya no es un `DELETE` real** — es borrado lógico vía `POST /api/panel-v2/papelera`, revierte `vehiculos.estado` y arrastra al expediente vinculado (son 1:1). Ver `app/panel/papelera/ARCHITECTURE.md`. No vuelvas a `supabase2.from("ventas").delete(...)` directo desde `VentasClient.tsx`.
