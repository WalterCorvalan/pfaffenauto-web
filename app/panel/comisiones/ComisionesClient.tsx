      ...
      <header className="flex flex-col border-b border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02] shrink-0 pt-6 px-6 pb-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 rounded-2xl p-4 flex flex-col justify-center">
            <span className="text-[10px] font-bold uppercase tracking-widest text-rose-500">Pendiente</span>
      ...
      <div className="flex-1 overflow-auto bg-slate-50 dark:bg-[#141414] p-6">
        <div className="bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden">
          {comisiones.length === 0 && !cargando ? (
            <p className="p-12 text-center text-slate-400 italic">No hay comisiones en este período.</p>
          ) : (
            <TablaResponsiva<any>
              filas={comisiones}
              keyExtractor={(c) => c.id}
              encabezadoMobile={(c) => (
                <div>
                  <p className="text-[13px] font-bold text-slate-800 dark:text-white">{c.beneficiario?.nombre}</p>
                  <span className="block text-[10px] text-slate-400 font-normal uppercase tracking-widest mt-0.5">{c.tipo.replace("_", " ")}</span>
                  <p className="text-[11px] text-slate-400 mt-0.5">{new Date(c.created_at).toLocaleDateString("es-AR")}</p>
                </div>
              )}
              columnas={
                [
                  { key: "fecha", header: "Fecha", cell: (c) => new Date(c.created_at).toLocaleDateString("es-AR"), claseTd: "text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap" },
                  { key: "beneficiario", header: "Beneficiario", cell: (c) => <>{c.beneficiario?.nombre}<span className="block text-[10px] text-slate-400 font-normal uppercase tracking-widest mt-0.5">{c.tipo.replace("_", " ")}</span></>, claseTd: "text-[13px] font-bold text-slate-800 dark:text-white", ocultarEnMobile: true },
                  { key: "concepto", header: "Concepto / Venta", anchoCompletoMobile: true, cell: (c) => (
                    <>
                      {c.tipo === "bono" ? (
                  ...
                  { key: "monto", header: "Monto Total", cell: (c) => <span className="font-mono text-[14px] font-bold text-emerald-600 dark:text-emerald-400">{c.moneda} {Number(c.monto).toLocaleString()}</span>, claseTd: "text-right" },
                  { key: "pagado", header: "Pagado / Restante", cell: (c) => {
                  ...
                  { key: "resenas", header: "Reseñas", cell: (c) => {
                    if (c.tipo === "bono") return <span className="text-slate-300 dark:text-slate-600">—</span>;
              ...
              acciones={(c) => esAdminOFinanzas && c.estado === "pendiente" && (
                <button
