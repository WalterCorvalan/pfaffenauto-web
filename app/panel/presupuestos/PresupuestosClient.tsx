      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
            <div>
              <h1 className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-2"><img src="/icons/panel/presupuestos.png" alt="" className="w-5 h-5 object-contain shrink-0" /> Presupuestos</h1>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Cotizaciones formales enviadas a clientes</p>
            </div>
            <button onClick={() => setModalAbierto(true)} className="flex items-center justify-center gap-1.5 bg-[#0145F2] hover:bg-[#0138c9] text-white px-4 py-2.5 rounded-xl text-sm font-bold transition-colors shrink-0"><Plus className="w-4 h-4" /> Nuevo Presupuesto</button>
          </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {([["todos", "Todos"], ["hoy", "Hoy"], ["7dias", "Últimos 7 días"], ["este_mes", "Este mes"], ["mes_pasado", "Mes pasado"]] as [Periodo, string][]).map(([key, label]) => (
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-lg text-[11px] font-bold transition-colors ${periodo === key && !desdeCustom && !hastaCustom ? "bg-[#0145F2] text-white" : "bg-slate-50 dark:bg-white/5 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10"}`}
                >
                  {label}
                </button>
              ))}
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                  {presupuestosPaginados.map((p: any) => (
                    <tr key={p.id} onClick={() => setSeleccionado(p)} className={`hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors border-l-4 cursor-pointer ${p.precio_confirmado === false ? "border-l-amber-400" : "border-l-rose-300"}`}>
                      <td className="px-4 py-3 font-mono text-[13px] font-bold text-[#0145F2] dark:text-[#5b8dff]">{p.numero || "—"}</td>
              ...
              <div key={p.id} onClick={() => setSeleccionado(p)} className={`bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl shadow-sm hover:shadow-md transition-shadow p-4 space-y-2 border-l-4 cursor-pointer active:bg-slate-50 dark:active:bg-white/[0.04] ${p.precio_confirmado === false ? "border-l-amber-400" : "border-l-rose-300"}`}>
                ...
                <div className="flex items-center justify-between text-[12px] text-slate-500 dark:text-slate-400 pt-1 border-t border-slate-100 dark:border-white/10" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <Link href={`/panel/presupuestos/imprimir/${p.id}`} className="inline-flex p-1.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg text-slate-400"><Printer className="w-3.5 h-3.5" /></Link>
                  </div>
                </div>
              </div>
            ))}
