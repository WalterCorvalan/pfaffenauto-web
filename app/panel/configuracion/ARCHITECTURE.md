...
- `resumen_diario_activo` — no tenía ningún proceso que generara el resumen, hasta que se creó el cron `resumen-empresa` (ver `app/api/cron/panel/resumen-empresa/route.ts`).
...
**`PermisosTab.tsx` se eliminó** — existía como pantalla completa ("Permisos por Rol" + "Excepciones por Usuario") pero nunca estuvo enlazada desde ningún tab de Configuración, así que era inalcanzable desde la UI y quedaba como código muerto. En su lugar, `ver_liquidacion` (el único permiso fino real que hay hoy) tiene un toggle chico y directo en **Configuración → Empresa → Módulos**, sección "Ver margen/ganancia" (checkbox por rol: encargado/ventas/finanzas/gestoria — admin siempre lo ve). Se guarda vía `PATCH /api/panel/modulos` con `tipo: "permiso_ver_liquidacion"`, mismo endpoint que ya manejaba módulos/visibilidad. Si en el futuro aparece un segundo permiso fino real, agregale su propio checkbox ahí en vez de reconstruir una pantalla genérica de permisos — hasta ahora ese patrón (una pantalla genérica desconectada de todo) fue justamente el problema.
