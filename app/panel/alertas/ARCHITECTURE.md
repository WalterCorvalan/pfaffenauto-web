...
5. **Reactivación de handoff viejo**: si una conversación quedó con `ai_habilitada=false` (el vendedor se hizo cargo) y no hubo ningún mensaje nuevo en 30+ días, vuelve a poner `ai_habilitada=true` y limpia `handoff_at`/`handoff_reason`/`handoff_resumen`. No borra historial — evita que la conversación quede muda para siempre si el vendedor nunca la cerró y el cliente vuelve a escribir mucho después.

El dedup es un flag boolean por fila (`aviso_caliente_sin_atender_enviado`, `aviso_agradecimiento_enviado`, `aviso_nudge_enviado`, `aviso_doc_pendiente_enviado` — ver la migración de columnas), mismo patrón que el resto de `seguimientos/route.ts`, no la tabla `automatizaciones_wa` aparte que usaba v1. La reactivación de handoff (5) no necesita flag de dedup propio: una vez reactivada, `ai_habilitada=true` y `handoff_at=null` ya la sacan de la condición del query.
