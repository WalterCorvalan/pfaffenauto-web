...
const PANEL_TABS = [
  ...
  { grupo: "PANEL", value: "mis-ventas", label: "Mis ventas", icon: Trophy, externo: "/panel/mis-ventas" },
...
export default function MiEspacioClient({
  ...
  useEffect(() => {
    ...
    if (guardada) { setTab(guardada); router.replace(`/panel/mi-espacio?tab=${guardada}`); }
  ...
  const irATab = (v: string) => {
    ...
    router.replace(`/panel/mi-espacio?tab=${v}`);
  };
  ...
  const renderTab = (t: any) => {
    ...
    if (t.externo) {
      return (
        <Link key={t.value} href={t.externo} className="px-3 py-1.5 text-xs font-bold whitespace-nowrap flex items-center gap-1.5 text-slate-500 dark:text-slate-400 hover:text-[#0145F2]">
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-start justify-between gap-3 mb-1 flex-wrap">
        ...
        <div className="flex items-center gap-2 shrink-0 relative">
          ...
          <button onClick={() => setMostrarAgregar((v) => !v)} className="flex items-center gap-1.5 px-4 py-2 text-sm font-bold bg-[#0145F2] hover:bg-[#0138c9] text-white rounded-lg"><Plus className="w-4 h-4" /> Agregar <ChevronDown className="w-3.5 h-3.5" /></button>
          {mostrarAgregar && (
            ...
            <div className="space-y-1.5">
              {RESUMEN_ITEMS.map((it) => (
                <label key={it.key} className="flex items-center gap-3 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-lg p-3 cursor-pointer">
                  <input type="checkbox" checked={items.includes(it.key)} onChange={(e) => togglePref(recibirResumen, e.target.checked ? [...items, it.key] : items.filter((k) => k !== it.key))} className="w-4 h-4 accent-[#0145F2]" />
      ...
      {tab === "pagos" && (
        <div>
          ...
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            <button onClick={() => setVerTodos((v) => !v)} className={`text-xs font-bold ${verTodos ? "text-[#0145F2]" : "text-slate-400 hover:text-[#0145F2]"}`}>Ver todos</button>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setPagando(null)}>
          <div onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-[#141414] border border-slate-200 dark:border-white/10 w-full max-w-sm rounded-2xl shadow-2xl p-6">
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button onClick={confirmarPago} disabled={guardandoPago} className="flex items-center gap-1.5 px-4 py-2 text-sm font-bold bg-[#0145F2] hover:bg-[#0138c9] text-white rounded-lg disabled:opacity-50"><Save className="w-4 h-4" /> Registrar pago</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={cerrarModalPagoManual}>
          <div onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-[#141414] border border-slate-200 dark:border-white/10 w-full max-w-md rounded-2xl shadow-2xl p-6">
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button onClick={registrarPagoManual} disabled={guardandoPagoManual} className="flex items-center gap-1.5 px-4 py-2 text-sm font-bold bg-[#0145F2] hover:bg-[#0138c9] text-white rounded-lg disabled:opacity-50"><Save className="w-4 h-4" /> {editandoPago ? "Guardar" : "Registrar"}</button>
            </div>
          </div>
        </div>
      )}
