    ...
    <div className="flex flex-col h-full w-full bg-white dark:bg-[#001233] overflow-hidden">
      <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 dark:border-[#0a2a6b] px-6 py-4 bg-white dark:bg-[#001c55] shrink-0 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[17px] font-bold text-slate-900 dark:text-white leading-tight">Responsabilidad Civil</h1>
            <p className="text-[11px] font-medium text-slate-500 dark:text-slate-400 mt-0.5">Recibos de entrega para venta a consumidor final</p>
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-[#0a2a6b]">
                  {registros?.map((r: any) => (
                    <tr key={r.id} className="hover:bg-purple-50/40 dark:hover:bg-[#00246b] transition-colors border-l-4 border-l-purple-300">
                      <td className="p-4 pl-6 font-mono text-[13px] font-bold text-indigo-600 dark:text-sky-300">{r.numero || "—"}</td>
                      ...
                      <td className="p-4 text-[13px] font-medium text-slate-900 dark:text-white">{r.apellido}, {r.nombre}</td>
                      ...
                      <td className="p-4 text-[13px] text-slate-500 dark:text-slate-400">{r.perfiles?.nombre || "—"}</td>
                      <td className="p-4 pr-6 text-center">
                        <Link href={`/panel/resp-civil/imprimir/${r.id}`} className="inline-flex p-2 bg-white dark:bg-[#00246b] hover:bg-indigo-50 dark:hover:bg-[#002a6e] border border-slate-200 dark:border-[#0a2a6b] hover:border-indigo-200 rounded-lg text-slate-400 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-sky-300 transition-all shadow-sm">
                  ...
                  {(!registros || registros.length === 0) && (
                    <tr>
                      <td colSpan={6} className="p-16 text-center text-slate-400 dark:text-slate-500 text-sm italic">
