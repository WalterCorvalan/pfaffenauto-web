...
export default function SucursalEditor({ autoId, sucursalActualId, sucursalActualNombre, sucursales, puedeGestionar }: SucursalEditorProps) {
  ...
  return (
    <>
      ...
      >
        <MapPin className={`w-3.5 h-3.5 shrink-0 ${puedeGestionar ? 'text-slate-400 group-hover:text-indigo-600' : 'text-slate-400'}`} />
          ...
          <div className="relative bg-white border border-slate-200 w-full max-w-sm rounded-2xl shadow-2xl p-6 animate-fadeIn">
            <div className="flex justify-between items-center mb-5 border-b border-slate-100 pb-3">
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                <MapPin className="w-5 h-5 text-indigo-600" /> Mover Vehículo
              </h3>
            ...
            <form onSubmit={guardarSucursal} className="space-y-4">
              <div>
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">Nueva Ubicación</label>
                <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl px-3 focus-within:border-indigo-500 transition-colors shadow-sm">
                  <select value={nuevaSucursal} onChange={(e) => setNuevaSucursal(e.target.value)} className="w-full bg-transparent py-3 text-sm text-slate-900 outline-none appearance-none cursor-pointer">
