    ...
    <div className="flex flex-col h-full w-full bg-white overflow-hidden">
      <header className="flex items-center justify-between border-b border-slate-200 px-6 py-4 bg-white shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[17px] font-bold text-slate-900 leading-tight">Categorías de Empleado</h1>
            <p className="text-[11px] font-medium text-slate-500 mt-0.5">Sueldo base y reglas para el liquidador de sueldos</p>
          </div>
        </div>
        <button
          onClick={abrirNueva}
            ...
            {categorias.map((c) => (
              <div key={c.id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                <div className="flex items-start justify-between mb-3">
                  <h3 className="font-bold text-slate-900 text-[15px]">{c.nombre}</h3>
                  <div className="flex items-center gap-1">
                    <button onClick={() => abrirEditar(c)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">
                      <Edit2 className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => eliminar(c.id, c.nombre)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors">
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
                </div>
                <p className="text-2xl font-black text-slate-900 mb-3">$ {c.sueldo_base.toLocaleString("es-AR")}</p>
                <div className="flex flex-wrap gap-1.5">
                  {c.tiene_comision && (
                    <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-md">
                      + Comisión de ventas
                    </span>
                  )}
                  {c.monto_por_auto_taller > 0 && (
                    <span className="bg-amber-50 text-amber-700 border border-amber-200 text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-md">
                      + $ {c.monto_por_auto_taller.toLocaleString("es-AR")} por auto (taller)
                    </span>
                  )}
                  {!c.tiene_comision && c.monto_por_auto_taller === 0 && (
                    <span className="bg-slate-50 text-slate-500 border border-slate-200 text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-md">
                      Sueldo fijo
                    </span>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white border border-slate-200 w-full max-w-md rounded-2xl shadow-2xl p-6 animate-fadeIn">
            <div className="flex justify-between items-center mb-5 border-b border-slate-100 pb-3">
              <h3 className="text-lg font-bold text-slate-900">{editando ? "Editar categoría" : "Nueva categoría"}</h3>
            ...
            <form onSubmit={guardar} className="space-y-4">
              <div>
                ...
                <input required value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej: Vendedor, Encargado, Taller" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm outline-none focus:border-indigo-500 focus:bg-white text-slate-900" />
              </div>
              <div>
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">Sueldo base ($) *</label>
                <input required type="number" value={sueldoBase} onChange={(e) => setSueldoBase(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm outline-none focus:border-indigo-500 focus:bg-white text-slate-900" />
              </div>
              <label className="flex items-center gap-3 text-sm text-slate-700 cursor-pointer bg-slate-50 border border-slate-200 p-3.5 rounded-xl hover:bg-white transition-colors font-medium">
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 block">Monto por auto trabajado (taller/detailing)</label>
                <input type="number" value={montoPorAuto} onChange={(e) => setMontoPorAuto(e.target.value)} placeholder="0 si no aplica" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm outline-none focus:border-indigo-500 focus:bg-white text-slate-900" />
              </div>
