    ...
    <div className="flex flex-col h-full w-full bg-white overflow-hidden">
      <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 px-6 py-4 bg-white shrink-0 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[17px] font-bold text-slate-900 leading-tight">Señas</h1>
            <p className="text-[11px] font-medium text-slate-500 mt-0.5">Anticipos y reservas de unidades</p>
          </div>
        </div>
                  ...
                  {senas?.map((s: any) => (
                    <tr key={s.id} className={`hover:bg-indigo-50/40 transition-colors border-l-4 ${COLOR_ESTADO[s.estado] || "border-l-slate-200"}`}>
                      ...
                      <td className="p-4 pr-6 text-center">
                        <Link href={`/panel/senas/imprimir/${s.id}`} className="inline-flex p-2 bg-white hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 rounded-lg text-slate-400 hover:text-indigo-600 transition-all shadow-sm">
