...
import { Printer, ArrowLeft, AlertTriangle, CheckCircle2, Copy } from "lucide-react";
      ...
      <div className="print:hidden max-w-[210mm] mx-auto mb-8 bg-white dark:bg-[#001c55] p-6 rounded-2xl border border-slate-200 dark:border-[#0a2a6b] shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {s.codigo_seguimiento && (
            <button
              onClick={() => navigator.clipboard.writeText(`${window.location.origin}/seguimiento/${s.codigo_seguimiento}`)}
              className="flex items-center gap-1.5 bg-slate-50 dark:bg-[#002a6e] hover:bg-slate-100 dark:hover:bg-[#0a2a6b] text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-[#0a2a6b] px-3 py-2 rounded-xl text-xs font-bold font-mono transition-colors"
              title="Copiar link de seguimiento para el cliente"
            >
              <Copy className="w-3.5 h-3.5" /> {s.codigo_seguimiento}
            </button>
          )}
