    ...
    <div className="flex flex-col h-full w-full bg-white dark:bg-[#001233] overflow-hidden">
      <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 dark:border-[#0a2a6b] px-6 py-4 bg-white dark:bg-[#001c55] shrink-0 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[17px] font-bold text-slate-900 dark:text-white leading-tight">Postventa</h1>
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-[#F9FAFB] dark:bg-[#001233] custom-scrollbar">
        <div className="max-w-[1400px] mx-auto">
          ...
          {recordatoriosPendientes.length > 0 && (
            <div className="mb-6">
              <h2 className="text-[11px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-3 flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {recordatoriosPendientes.map((r) => (
                  <div key={r.ventaId} className="bg-amber-50 dark:bg-[#002a6e] border border-amber-200 dark:border-[#0a2a6b] rounded-xl p-4 shadow-sm">
                    <h3 className="font-bold text-[14px] text-slate-900 dark:text-white mb-1">{r.cliente?.nombre || "Sin nombre"}</h3>
                    ...
                    <p className="text-[11px] text-slate-600 dark:text-slate-300 font-medium flex items-center gap-1 mb-3">
                      <CarFront className="w-3 h-3 text-slate-400 dark:text-slate-500" /> {r.vehiculo?.marca} {r.vehiculo?.modelo}
                    </p>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {r.cliente?.telefono_celular && (
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 bg-white dark:bg-[#001c55] hover:bg-emerald-50 dark:hover:bg-[#00246b] border border-emerald-200 dark:border-[#0a2a6b] text-emerald-700 dark:text-emerald-300 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-colors"
                        >
                          WhatsApp
                        </a>
                      )}
