...
import { Search, Send, CheckCircle2, Circle, Bot, Check, Info, ChevronRight, PanelRight, Loader2 } from "lucide-react";

const ETAPAS_PIPELINE = ["Nuevo", "En conversación", "Interesado", "Cliente", "Perdido"];

export default function ChatClient({ conversacionesIniciales }: { conversacionesIniciales: any[] }) {
  ...
  const formatDay = (dateString: string) => {
    const date = new Date(dateString);
    const difDays = Math.floor((new Date().getTime() - date.getTime()) / (1000 * 3600 * 24));
  ...
  return (
    <div className="flex w-full h-full text-slate-800">
      ...
      <div className="w-full md:w-[320px] flex flex-col bg-white border-r border-slate-200 shrink-0">
        <div className="p-4 border-b border-slate-100 flex-shrink-0">
          <h2 className="text-[17px] font-bold text-slate-900 mb-4 flex items-center gap-2">
            Bandeja <span className="text-sm font-normal text-slate-400">{conversaciones.length}</span>
          </h2>
          ...
          <div className="flex gap-2">
            ...
            >
              No leídas <span className="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-full">0</span>
            </button>
          </div>
        </div>
            ...
            return (
              <button
                ...
                className={`w-full text-left p-3.5 border-b border-slate-100 transition-all flex gap-3 ${
                  isActive ? "bg-emerald-50 border-l-2 border-l-emerald-700" : "bg-white border-l-2 border-l-transparent hover:bg-slate-50"
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-0.5">
                    <span className={`font-bold text-sm truncate ${isActive ? "text-emerald-900" : "text-slate-900"}`}>
      ...
      <div className={`flex-1 flex flex-col bg-[#F0F2F5] relative ${!seleccionada ? "hidden md:flex" : "flex"}`}>
        {!seleccionada ? (
          ...
          <>
            ...
            <div className="h-[60px] bg-white border-b border-slate-200 flex justify-between items-center px-6 shrink-0 shadow-sm z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 font-medium">{contactoActivo?.telefono}</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-3 custom-scrollbar">
              {loading ? (
                <div className="flex justify-center py-4"><span className="text-xs text-slate-500">Cargando...</span></div>
              ) : (
                mensajes.map((m) => {
            ...
            <div className="p-4 bg-[#F0F2F5]">
              <form onSubmit={enviarMensaje} className="flex items-center gap-3 max-w-4xl mx-auto bg-white border border-slate-200 rounded-xl px-2 py-1.5 shadow-sm">
                <input
                  type="text"
                  value={nuevoMensaje}
      ...
      {seleccionada && panelAbierto && (
        <div className="w-[300px] bg-white border-l border-slate-200 flex-col hidden lg:flex shrink-0 transition-all duration-300">
          ...
          <div className="h-[60px] p-4 border-b border-slate-200 flex items-center justify-between shrink-0">
            ...
            <button onClick={() => setPanelAbierto(false)} className="text-slate-400 hover:text-slate-700 p-1 rounded-md hover:bg-slate-100 transition-colors">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            ...
            <div className="p-6 border-b border-slate-200 flex flex-col items-center text-center">
              ...
              <h3 className="font-bold text-slate-900 text-lg">{contactoActivo?.nombre_perfil || "Cliente"}</h3>
              <p className="text-sm text-slate-500 mt-1">{contactoActivo?.telefono}</p>
            </div>
            ...
            <div className="p-6 border-b border-slate-200">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h4 className="text-sm font-bold text-slate-900">IA en esta conversación</h4>
                  <p className="text-[11px] text-slate-500 mt-0.5">{conversacionActiva?.ai_habilitada ? "Activada" : "Pausada"}</p>
                </div>
                <div className={`w-11 h-6 rounded-full flex items-center px-1 cursor-pointer transition-colors ${conversacionActiva?.ai_habilitada ? "bg-emerald-700 justify-end" : "bg-slate-300 justify-start"}`}>
              ...
              {!conversacionActiva?.ai_habilitada && (
                <div className="bg-amber-50 border border-amber-200 p-3 rounded-lg flex gap-2">
                  ...
                  <p className="text-xs text-amber-800 leading-relaxed">Falta la clave de IA de la instancia o intervino un humano.</p>
                </div>
              )}
            </div>
            ...
            {/* Etapa del Pipeline */}
            <div className="p-6 border-b border-slate-200">
              <h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-4">Etapa del Pipeline</h4>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-[13px] transition-colors ${
                        ...
                        isCompleted ? "text-slate-700 font-medium" : "text-slate-400 group-hover:text-slate-600"
                      }`}>
                        {etapa}
                      </span>
                    </div>
