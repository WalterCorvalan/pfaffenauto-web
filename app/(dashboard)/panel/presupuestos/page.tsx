    ...
    <div className="flex flex-col h-full w-full bg-white overflow-hidden">
      <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 px-6 py-4 bg-white shrink-0 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[17px] font-bold text-slate-900 leading-tight">Presupuestos</h1>
            <p className="text-[11px] font-medium text-slate-500 mt-0.5">Cotizaciones formales enviadas a clientes</p>
          </div>
        </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {presupuestos?.map((p: any) => (
                    <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="p-4 text-[13px] text-slate-500">{p.perfiles?.nombre || "—"}</td>
                      ...
                      <td className="p-4 pr-6 text-center">
                        <Link href={`/panel/presupuestos/imprimir/${p.id}`} className="inline-flex p-2 bg-white hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 rounded-lg text-slate-400 hover:text-indigo-600 transition-all shadow-sm">
                  ...
                  {(!presupuestos || presupuestos.length === 0) && (
                    <tr>
                      <td colSpan={7} className="p-16 text-center text-slate-400 text-sm italic">
