    ...
    <div className="flex flex-col h-full w-full bg-white overflow-hidden">
      <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 px-6 py-4 bg-white shrink-0 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[17px] font-bold text-slate-900 leading-tight">Asistente Virtual</h1>
            <p className="text-[11px] font-medium text-slate-500 mt-0.5">Qué le pregunta la gente al chatbot de la web</p>
          </div>
        </div>
        ...
        <div className="max-w-[1200px] mx-auto space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex items-center gap-2">
              ...
              <h2 className="text-[11px] font-bold uppercase tracking-widest text-slate-500">Preguntas más repetidas</h2>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {rankingPreguntas.map(([pregunta, stats]) => (
                    <tr key={pregunta} className="hover:bg-slate-50/50 transition-colors">
                      <td className="p-4 pl-6 font-medium text-[13px] text-slate-800 max-w-[500px] truncate" title={pregunta}>{pregunta}</td>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex items-center gap-2">
              ...
              <h2 className="text-[11px] font-bold uppercase tracking-widest text-slate-500">
                Preguntas que el bot no supo responder <span className="text-slate-400 normal-case font-medium">— usalas para mejorar el guion</span>
              </h2>
            </div>
            <div className="divide-y divide-slate-100 max-h-[350px] overflow-y-auto custom-scrollbar">
              {datos.filter((p) => !p.respondida).slice(0, 30).map((p, i) => (
                <div key={i} className="flex items-center justify-between px-6 py-3 text-[13px]">
                  <span className="font-medium text-slate-700">{p.pregunta}</span>
              ...
              {datos.filter((p) => !p.respondida).length === 0 && (
                <div className="p-10 text-center text-slate-400 text-sm italic">El bot respondió todo. 🎉</div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex items-center gap-2">
              ...
              <h2 className="text-[11px] font-bold uppercase tracking-widest text-slate-500">Últimas preguntas</h2>
            </div>
            <div className="divide-y divide-slate-100 max-h-[400px] overflow-y-auto custom-scrollbar">
              {datos.slice(0, 30).map((p, i) => (
                <div key={i} className="flex items-center justify-between px-6 py-3 text-[13px]">
                  <span className="font-medium text-slate-700 truncate max-w-[600px]" title={p.pregunta}>{p.pregunta}</span>
                  <div className="flex items-center gap-3 shrink-0">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${p.respondida ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}>
