...
export default async function AutosPautadosPage() {
  ...
  const badgeEstado = (estado: string) => {
    switch (estado) {
      case "Disponible": return "bg-emerald-50 dark:bg-[#002a6e] text-emerald-600 dark:text-emerald-300 border-emerald-200 dark:border-[#0a2a6b]";
      case "Reservado": return "bg-amber-50 dark:bg-[#002a6e] text-amber-600 dark:text-amber-300 border-amber-200 dark:border-[#0a2a6b]";
      case "Vendido": return "bg-slate-100 dark:bg-[#00246b] text-slate-500 dark:text-slate-400 border-slate-200 dark:border-[#0a2a6b]";
      default: return "bg-slate-50 dark:bg-[#00246b] text-slate-500 dark:text-slate-400 border-slate-200 dark:border-[#0a2a6b]";
    }
  };
    ...
    <div className="flex flex-col h-full w-full bg-white dark:bg-[#001233] overflow-hidden">
      <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 dark:border-[#0a2a6b] px-6 py-4 bg-white dark:bg-[#001c55] shrink-0 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[17px] font-bold text-slate-900 dark:text-white leading-tight">Autos Pautados</h1>
            <p className="text-[11px] font-medium text-slate-500 dark:text-slate-400 mt-0.5">Unidades con inversión publicitaria activa</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 bg-orange-50 dark:bg-[#002a6e] border border-orange-200 dark:border-[#0a2a6b] px-3 py-1.5 rounded-lg text-[11px] font-bold text-orange-700 dark:text-orange-300">
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-[#F9FAFB] dark:bg-[#001233] custom-scrollbar">
        <div className="max-w-[1200px] mx-auto space-y-6">
          ...
          {canalesOrdenados.length > 0 && (
            <div className="bg-white dark:bg-[#001c55] border border-slate-200 dark:border-[#0a2a6b] rounded-2xl shadow-sm p-6">
              <h2 className="text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-4">Distribución por canal de pauta</h2>
              <div className="flex flex-wrap gap-3">
                {canalesOrdenados.map(([canal, cantidad]) => (
                  <div key={canal} className="flex items-center gap-2 bg-slate-50 dark:bg-[#00246b] border border-slate-200 dark:border-[#0a2a6b] px-4 py-2 rounded-xl">
                    <span className="font-bold text-[13px] text-slate-800 dark:text-white">{canal}</span>
                    <span className="bg-white dark:bg-[#001c55] border border-slate-200 dark:border-[#0a2a6b] text-[11px] font-bold text-indigo-600 dark:text-sky-300 px-2 py-0.5 rounded-md">{cantidad}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-[#0a2a6b]">
                  {activos.map((v: any) => (
                    <tr key={v.id} className="hover:bg-slate-50/50 dark:hover:bg-[#00246b] transition-colors">
                      <td className="p-4 pl-6">
                        <Link href={`/panel/vehiculo/editar/${v.id}`} className="flex items-center gap-3 group">
                          {v.multimedia_vehiculos?.[0] ? (
                            ...
                            <div className="w-14 h-10 bg-slate-100 dark:bg-[#00246b] border border-slate-200 dark:border-[#0a2a6b] rounded-md flex items-center justify-center shrink-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <span className="font-bold text-[13px] text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-sky-300 transition-colors block truncate">{v.marca} {v.modelo}</span>
                          </div>
                        </Link>
                      </td>
